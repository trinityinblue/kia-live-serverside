...
state_queue.put({'last_update': datetime.now(), 'data': {}})
updating = Queue(1)
...
def main_runner():
            ...
            for key in req.keys():
                ...
                if 'data' in req[key].keys() and len(req[key]['data']) > 0:
                    ...
                    old_full_data = state_queue.get().copy() if not state_queue.empty() else \
                        {'last_update': datetime.now(), 'data': {}}
                    state_queue.put(old_full_data)
                    old_data = old_full_data['data']
                    old_data[name] = {'pollDate': datetime.now().isoformat()}
    ...
    async def update_loop(q: Queue):  # Update data_snapshot very frequently, call update_loop_call
        try:
            ...
            # Main loop
            data_snapshot = state_queue.get().copy() if not state_queue.empty() else {'last_update': datetime.now(),
                                                                                      'data': {}}
            ...
            while data_snapshot['last_update'] < (datetime.now() + timedelta(minutes=20)):
                #
                ...
                if update_state and 'time' in next_update.keys() and next_update['time'] < datetime.now():
                ...
                for response_data in response['data']:
                    ...
                    if 'tripdetails' in response_data.keys() and len(response_data['tripdetails']) > 0:
                        ...
                        for trip in response_data['tripdetails']:
                            ...
                            original_start = datetime.strptime(f"{tomorrow_str} {trip['starttime']}",
                                                               '%Y-%m-%d %H:%M')
                            for i in range(5):
                                buffer = i * 5
